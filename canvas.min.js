(function(window) {
	! function(e) {
		...
		var n, t = 0,
			a = ["moz", "webkit", "o", "ms"];
		for (n = 0; n < a.length && !e.requestAnimationFrame; n += 1) e.requestAnimationFrame =
			e[a[n] + "RequestAnimationFrame"], e.cancelAnimationFrame = e[a[n] +
				"CancelAnimationFrame"] || e[a[n] + "CancelRequestAnimationFrame"];
		e.cancelAnimationFrame || (e.requestAnimationFrame ? (a = e.requestAnimationFrame,
			t = {}, e.requestAnimationFrame = function(e) {
				var i = n;
	...
	window.CanvasRenderingContext2D && (CanvasRenderingContext2D.prototype.reset =
		function() {
	...
	window.Canvas = class Canvas {
		constructor() {
			this.id = arguments[0];
			this.element = document.getElementById(this.id) ? document.getElementById(
				this.id) : document.createElement('canvas');
            this.element.style.backgroundColor = 'white';
			this.element.id = this.id;
			this.renderer = this.element.ctx = this.element.getContext(arguments[3] ||
		...
		on(event, fn) {
			...
			this.eventListeners.set(event, fn); // this.eventListenerというMapインスタンスに，keyをevent，valueをfnとして保存
			return this // method chainと呼ばれるらしい instance.method1.method2みたいにできる
		}
		fireEvent(event) {
			let args = [];
			Array.prototype.push.apply(args, arguments);
			args.shift();
			if (this.eventListeners.has(event)) this.eventListeners.get(event).apply(
				this, args)
		}
		drawWorkingFrame() {
		...
		_loop() {
			...
			this._lastFrame = now;
			if (!this._paused) this.fireEvent('update', this._HANDYOBJECT_, delta,
				Canvas.now());
		...
		loadObject(key) {
			key = key || 'HO';
			const handyObject = JSON.parse(window.localStorage.getItem('canvas-' +
				key));
		...
		renderCanvasElements() {
			...
			for (let element of this.canvasElements) {
				if (element.animation) {
					const ratio = (Canvas.now() - element.animation.start) / (element.animation
						.end - element.animation.start);
					if (ratio >= 1) {
						element.animation = !1
					} else {
						switch (element.animation.type) {
							case 'move':
								element.x = Canvas.lerp(element.animation.from.x, element.animation.to
									.x, ratio);
								element.y = Canvas.lerp(element.animation.from.y, element.animation.to
									.y, ratio);
								break;
							case 'lineLength':
								element.lineLength = Canvas.lerp(element.animation.from.lineLength,
									element.animation.to.lineLength, ratio)
						}
					}
				}
				switch (element.constructor.name) {
					case 'CanvasImage':
						...
						if (element.sourceX)
							renderer.drawImage(element.element, element.sourceX, element.sourceY,
								element.sourceWidth, element.sourceHeight, element.x, element.y,
								element.width, element.height);
						else renderer.drawImage(element.element, element.x, element.y, element
					...
					case 'CanvasLine':
						...
						renderer.lineTo(Canvas.lerp(element.from.x, element.to.x, element.lineLength),
	...
	Canvas.Element = class Element {
		constructor() {
			...
			this.rotation = typeof args.rotation !== 'undefined' ? args.rotation :
				null;
			this.animation = !1
		}
		animate() {
